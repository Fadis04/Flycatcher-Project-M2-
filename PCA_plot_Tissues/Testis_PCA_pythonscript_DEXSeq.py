...
metadata_file = "/home/sfadi/Bureau/PCA_plot_Tissues/testis_grp_file"
...
# Filter metadata for testis tissue
metadata_testis = metadata[metadata['Tissue'] == 'Testis']
...
filtered_metadata = pd.DataFrame(columns=metadata_testis.columns)

for species, count in species_count.items():
    species_subset = metadata_testis[metadata_testis['Species'] == species].head(count)
    filtered_metadata = pd.concat([filtered_metadata, species_subset])

# Transpose proportions data to have samples as rows and features as columns
proportions_data = proportions_data.T

# Merge metadata with proportions data based on 'Sample'
merged_data = pd.merge(filtered_metadata, proportions_data, left_on='Sample', right_index=True, how='inner')
...
plt.title('PCA Plot: Exon Usage Variation in Testis Tissue', fontsize=20)

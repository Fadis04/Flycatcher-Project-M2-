...
bam_path <- "/beegfs/data/fslimi/fic_bam/fic_hyp_bam/"
...
read_counts_fic_hyp <- assay(se)

# Save read counts to a file in the desired path
write.table(read_counts_fic_hyp , file = "/beegfs/data/fslimi/read_counts_fic_hyp.txt", sep = "\t", quote = FALSE, col.names = TRUE, row.names = TRUE)
